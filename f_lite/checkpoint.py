# copied from https://github.com/pytorch/examples/blob/main/distributed/FSDP2/checkpoint.py
...
SAMPLER_STATE = "sampler_state.pt"
PARAMS = "params"
...
class Checkpointer:
    ...
    def load_model(self, model: FSDPModule, iteration=None, checkpoint_path=None):
        if iteration is None and checkpoint_path is None:
            iteration = self.last_training_time
        last_model_checkpoint = checkpoint_path or (
            f"{self.folder}/{'dcp_api' if self.dcp_api else 'dtensor_api'}"
            f"/{iteration}"
        )
        last_model_checkpoint = os.path.join(last_model_checkpoint, MODEL_CHECKPOINT)
        full_sd = torch.load(
    ...
    def load_optim(self, model: FSDPModule, opt: torch.optim.Optimizer, iteration=None, checkpoint_path=None):
        if iteration is None and checkpoint_path is None:
            iteration = self.last_training_time
        last_optim_checkpoint = checkpoint_path or (
            f"{self.folder}/{'dcp_api' if self.dcp_api else 'dtensor_api'}"
            f"/{iteration}"
        )
        last_optim_checkpoint = os.path.join(last_optim_checkpoint, OPTIM_CHECKPOINT)
        full_sd = torch.load(
    ...
    def save(self, iteration: int, model: FSDPModule, optim: torch.optim.Optimizer, sampler_state: dict):
        model_state_dict = self._get_full_model_state_dict(model)
        optim_state_dict = self._get_full_optimizer_state_dict(model, optim)
        if torch.distributed.get_rank() == 0:
            new_checkpoint_folder = f"{self.folder}/{'dcp_api' if self.dcp_api else 'dtensor_api'}/{iteration}"
            ...
            torch.save(sampler_state, f"{new_checkpoint_folder}/{SAMPLER_STATE}")